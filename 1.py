...
# === Расчет расстояний (косинусное сходство) ===
def compute_distances(embeddings1, embeddings2):
    similarities = cosine_similarity(embeddings1, embeddings2.reshape(1, -1))
    distances = 1 - similarities.flatten()
    return distances

# === Предположим, у нас уже есть база изображений и их эмбеддинги ===
# database_image_paths = [...]  # список путей
# database_embeddings = np.array([...])  # массив эмбеддингов размером [N, D]
from glob import glob
import os

# Папка с базой изображений
# Функция для получения эмбеддинга одного изображения
def get_embedding(image_path, model, transform, device):
    image = Image.open(image_path).convert('RGB')
    image = transform(image).unsqueeze(0).to(device)
    with torch.no_grad():
...
# Функция для подсчета косинусных расстояний
def compute_distances(embeddings1, embedding2):
    similarities = cosine_similarity(embeddings1, embedding2.reshape(1, -1))
    distances = 1 - similarities.flatten()
    return distances

# Папка с базой изображений (train)
database_dir = "C:/klasss/archive/newts/model_final/train"
database_image_paths = glob(os.path.join(database_dir, "**", "*.jpg"), recursive=True)
database_image_paths += glob(os.path.join(database_dir, "**", "*.png"), recursive=True)

print(f"Извлечение эмбеддингов из базы ({len(database_image_paths)} изображений)...")

# Вычисляем эмбеддинги базы
database_embeddings = []
for path in tqdm(database_image_paths, desc="Embedding Database"):
    emb = get_embedding(path, model, train_transforms, device)
    database_embeddings.append(emb)

database_embeddings = np.array(database_embeddings)

# === Поиск топ-5 похожих ===
new_image_path = "C:/klasss/archive/newts/model_final/finish/21-4-jan.JPG"  # путь к новому изображению
new_embedding = get_embedding(new_image_path, model, train_transforms, device)

distances = compute_distances(database_embeddings, new_embedding)
top5_idx = np.argsort(distances)[:5]
top5_paths = [database_image_paths[i] for i in top5_idx]

print("\n=== Top-5 Похожие Изображения ===")
for path in top5_paths:
    class_name = os.path.basename(os.path.dirname(path))
    print(f"Класс: {class_name} | Путь: {path}")


output_dir = "C:/klasss/archive/newts/model_final/top5_results"
os.makedirs(output_dir, exist_ok=True)

for i, path in enumerate(top5_paths):
    filename = f"top{i+1}_" + os.path.basename(path)
    destination = os.path.join(output_dir, filename)
    shutil.copy(path, destination)

print(f"\nTop-5 изображения сохранены в: {output_dir}")