...
async def get_embedding(image_path, model, transform, device):
    try:
...
async def compute_distances(embeddings1, embedding2):
    similarities = cosine_similarity(embeddings1, embedding2.reshape(1, -1))
    distances = 1 - similarities.flatten()
    return distances


async def save_embeddings(embeddings, paths, save_path):
    with open(save_path, 'wb') as f:
        pickle.dump({'embeddings': embeddings, 'paths': paths}, f)


async def load_embeddings(save_path):
    with open(save_path, 'rb') as f:
        data = pickle.load(f)
    return data['embeddings'], data['paths']


async def find_similar_images(model_path, database_dir, query_image_path, output_dir, transform, device):
    ...
    if os.path.exists(embeddings_save_path):
        ...
        database_embeddings, database_image_paths = await load_embeddings(embeddings_save_path)
    else:
        ...
        for path in tqdm(database_image_paths, desc="Обработка базы"):
            emb = await get_embedding(path, model, transform, device)
            if emb is not None:
                database_embeddings.append(emb)
                valid_paths.append(path)

        database_embeddings = np.array(database_embeddings)
        await save_embeddings(database_embeddings, valid_paths, embeddings_save_path)
        print(f"Эмбеддинги сохранены в {embeddings_save_path}")
        database_image_paths = valid_paths

    # Обработка запросного изображения
    model = await load_model(model_path, device)
    query_embedding = await get_embedding(query_image_path, model, transform, device)
    if query_embedding is None:
        print("Не удалось обработать запросное изображение")
        return

    # Поиск похожих изображений
    distances = await compute_distances(np.array(database_embeddings), query_embedding)
    top5_idx = np.argsort(distances)[:5]

    print("\n=== Топ-5 результатов ===")
    os.makedirs(output_dir, exist_ok=True)
    file = open('/home/yakshka/prod2/send_photo/res.txt', 'w')
    ...
    for i, idx in enumerate(top5_idx, 1):
        ...
        try:
            ...
            res_str = f"{i}. Класс: {class_string} | Особь: {individual}\n"
            file.write(res_str)
            print(f"{i}. Класс: {class_string} | Особь: {individual} | Путь: {src_path}")
        except Exception as e:
